...
def rook_endgame_transfer_pre_test():
    chess_mlp = ChessMLP()
    game_player = GamePlayer(chess_mlp)
    game_player.set_up(
        load_weights=True,
    ...
    rook_endgame_game = RookEndgameGame(
        ...
        train_steps=10,
        lambda_value=0.9,
        name="rook_endgame_transfer_pre_test"
    )
    game_player.play_game(
        game=rook_endgame_game,
        iterations=100,
...
def rook_endgame_transfer_post_test():
    chess_mlp = ChessMLP()
    game_player = GamePlayer(chess_mlp)
    game_player.set_up(
        load_weights=True,
    ...
    rook_endgame_game = RookEndgameGame(
        ...
        train_steps=10,
        lambda_value=0.9,
        name="rook_endgame_transfer_post_test"
    )
    game_player.play_game(
        game=rook_endgame_game,
        iterations=100,
...
def main():
    experiments = {
        ...
        "4" : rook_endgame_transfer_pre_test,
        "5" : rook_endgame_transfer_post_test
    }
